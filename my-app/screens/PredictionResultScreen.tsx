...
const PredictionResultScreen: React.FC<PredictionResultScreenProps> = ({
  ...
  const predictTime = async (forecast: ForecastData | undefined) => {
    ...
    console.log("Sending to ML Model:", payload);
  ...
  // Removed duplicate declaration of getWaterRecommendation
...
{/* <View style={styles.card}>
  <Text style={styles.cardTitle}>Forecast Overview</Text>
  <Text>
    Now: {mountain.forecastNow?.temp}°C, {mountain.forecastNow?.humidity}%, {mountain.forecastNow?.weather}
  </Text>
  <Text style={styles.advice}>
    {getHumidityAdvice(mountain.forecastNow?.humidity ?? 0)}
  </Text>

  <Text>
    In 3h: {mountain.forecast3h?.temp}°C, {mountain.forecast3h?.humidity}%, {mountain.forecast3h?.weather}
  </Text>
  <Text style={styles.advice}>
    {getHumidityAdvice(mountain.forecast3h?.humidity ?? 0)}
  </Text>

  <Text>
    In 6h: {mountain.forecast6h?.temp}°C, {mountain.forecast6h?.humidity}%, {mountain.forecast6h?.weather}
  </Text>
  <Text style={styles.advice}>
    {getHumidityAdvice(mountain.forecast6h?.humidity ?? 0)}
  </Text>
</View> */}
